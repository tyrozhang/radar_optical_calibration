...
| `ElevationConvention.java` | **俯仰角符号约定枚举**。定义 UP_NEGATIVE（向上为负，内部默认）和 UP_POSITIVE（向上为正）两种约定，提供 apply() 方法按设备约定转换俯仰角符号 |
...
```json
{
  "parameters": {
    ...
    "dAz_radar": 0.055442,
    "dEl_radar": 0.02155,
    "elevation_convention": "up_negative",
    "radar2opticalTotalDelayMs": 0
  }
}
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `radar_blh` | object | 是 | 雷达站 BLH 坐标 |
| `optical_blh` | object | 是 | 光电站 BLH 坐标 |
| `dAz0` | double | 是 | 方位角固定偏差（度） |
| `dEl0` | double | 是 | 俯仰角固定偏差（度） |
| `radar_compensation_type` | string | 是 | 雷达补偿类型：`"fixed"` 固定补偿 / `"segmented"` 分段补偿 |
| `dAz_radar` / `dEl_radar` | double | 固定补偿时必填 | 雷达固定补偿值（度） |
| `radar_compensation` | object | 分段补偿时必填 | 雷达分段补偿配置 |
| `elevation_convention` | string | 否 | 俯仰角符号约定，默认 `"up_negative"`（详见下方说明） |
| `radar2opticalTotalDelayMs` | long | 否 | 雷达到光电的总时延（毫秒），默认 `0` |

## 俯仰角符号约定（ElevationConvention）

### 背景问题

不同光电设备对俯仰角正负方向的定义不同：

- **部分设备**：向上（仰视）为负，向下（俯视）为正
- **部分设备**：向上（仰视）为正，向下（俯视）为负

如果系统内部计算使用的约定与设备约定不一致，会导致俯仰角方向反转，光电指向错误方向。

### 解决方案

系统内部统一使用 **UP_NEGATIVE**（向上为负）约定进行计算，在输出 `TrackingCommand` 时根据设备约定自动转换符号。

### 配置方式

在 `calibration_v2.json` 的 `parameters` 段中设置 `elevation_convention` 字段：
...
| 配置值 | 含义 | 适用场景 |
|--------|------|----------|
| `"up_negative"` | 向上（仰视）为负，向下（俯视）为正 | 内部默认约定，向后兼容。若不配置此字段，默认使用此约定 |
| `"up_positive"` | 向上（仰视）为正，向下（俯视）为负 | 俯仰角正方向向上的设备 |

> **注意**：若不设置 `elevation_convention` 字段，系统默认使用 `"up_negative"`，完全向后兼容。

### 转换规则

| 内部计算值 | UP_NEGATIVE（默认） | UP_POSITIVE |
|-----------|-------------------|-------------|
| 仰视 30° | **-30°** | **+30°** |
| 俯视 30° | **+30°** | **-30°** |
| 水平 0° | **0°** | **0°** |
...
`elevation_convention` 影响以下输出字段：

| 字段 | 是否受影响 | 说明 |
|------|----------|------|
| `TrackingCommand.elCmd` | ✅ 是 | 已按设备约定转换 |
| `TrackingCommand.elGeo` | ✅ 是 | 已按设备约定转换 |
| `TrackingCommand.dEl0` | ❌ 否 | 补偿值始终使用内部约定，与设备无关 |
| `TrackingCommand.dElRadar` | ❌ 否 | 补偿值始终使用内部约定，与设备无关 |

### 代码中使用

```java
// 从配置文件加载（自动读取 elevation_convention）
RadarOpticTrackerV2 tracker = new RadarOpticTrackerV2("calibration_v2.json");

// 或手动指定约定
ElevationConvention convention = ElevationConvention.UP_POSITIVE;
double elDevice = convention.apply(elInternal);  // 将内部俯仰角转换为设备约定

// 解析配置字符串
ElevationConvention conv = ElevationConvention.fromString("up_positive");  // → UP_POSITIVE
ElevationConvention conv2 = ElevationConvention.fromString(null);          // → UP_NEGATIVE（默认）
```
