 ...
 * - 焦距：focalLength（mm，0.0 表示无效/未配置焦距表）
 ...
 *   （补偿值在内部约定下标定，与设备无关）
 * - focalLength：基于3D斜距查表得到，距离基准与雷达补偿分段的 haversine 水平距离不同
 */
public record TrackingCommand(
    ...
    double opticalToTargetRange,// 光电到目标3D斜距（米）
    double targetWidth,         // 目标宽（米）
    double targetHeight,        // 目标高（米）
    double focalLength,         // 焦距（mm），0.0 表示无效
    ...
    public TrackingCommand(
        ...
        this(azCmd, elCmd, azGeo, elGeo, dAz0, dEl0, dAzRadar, dElRadar,
             opticalToTargetRange, targetWidth, targetHeight, focalLength, 0L);
    }
    ...
    public TrackingCommand(
        ...
        this(azCmd, elCmd, azGeo, elGeo, dAz0, dEl0, dAzRadar, dElRadar,
             opticalToTargetRange, targetWidth, targetHeight, 0.0, 0L);
    }
}
